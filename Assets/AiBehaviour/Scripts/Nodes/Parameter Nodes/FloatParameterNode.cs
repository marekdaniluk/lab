...
namespace AiBehaviour {
    ...
    public class FloatParameterNode : AParameterNode<float> {
        ...
        public override bool Run() {
            switch (_condition) {
                case FloatCondition.Greater: {
                    if (Blackboard.FloatParameters[Key] > (DynamicValue ? Blackboard.FloatParameters[Key] : Value)) {
                        return true;
                    }
                    break;
                }
        ...
        public override string ToString() {
            return string.Format("Is {0} {1} from {2}?", Blackboard.FloatParameters[Key], Condition.ToString(), (DynamicValue ? Blackboard.FloatParameters[Key] : Value));
        }
    }
}
